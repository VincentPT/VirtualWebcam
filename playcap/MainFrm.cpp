...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWnd)
	ON_WM_CREATE()
	ON_WM_SETFOCUS()
    ON_COMMAND(ID_VIEW_CAMERAS, &CMainFrame::OnViewCameras)
    ON_WM_SIZE()
    ON_WM_CLOSE()
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
    ...
    // init window handle for video app
    ghApp = m_wndView.m_hWnd;
...
void CMainFrame::OnViewCameras()
{
    ...
    if (captureDevice.DoModal() == IDOK) {
        ...
        if (device) {
            CaptureVideo(device, captureMode, selectedFormat);
        }
    }
}
